...
The example below uses correct settings for STM32L4R9I-EVAL board: 
  - SDMMC1 Mode:             SD 4bits Wide bus with dir voltage converter
  - Card Detect Input pin:   not available
...
static int32_t PowerControl (ARM_POWER_STATE state, MCI_RESOURCES *mci) {
  ...
  switch (state) {
    ...
    case ARM_POWER_FULL:
      ...
      mci->info->ker_clk = HAL_RCCEx_GetPeriphCLKFreq(RCC_PERIPHCLK_SDMMC1);
